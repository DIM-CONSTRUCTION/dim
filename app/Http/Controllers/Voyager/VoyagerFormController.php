...
class VoyagerFormController extends VoyagerBaseController
{
    ...
    public function duplicateFields(Request $request,$slug)
    {
             ...
             foreach ($ids as $id) {
                 $field = $model::where('id',$id)->first();
                 $new_field  = $field->replicate();
                 $new_field->project_id = $request->project_id;
                 $new_field->save();
    ...
    public function getDivisions(Request $request)
    {
      ...
      $divisionArray = self::getFunction($form->project_id,'project_divisions',0);
      return response(['divisions'=>$divisionArray]);
    }
        ...
        $areaList = DB::table($type)->where('project_id',$area->project_id)->where('parent_id',$area->id)->get();
          ...
          if(count($areaList) == 0)
          {
            ...
            if($type == 'areas')
            {
                ...
                $areaArray['is_percentage'] = $area->is_percentage;
            }
      ...
      foreach ($list as $key => $item) {
          ...
          if(count($areaList) == 0)
          {
            ...
            if($type == 'areas')
            {
                ...
                $areaArray['is_percentage'] = $item->is_percentage;
            }
    ...
    //  $activityArray['division'] = $request->params['division'];
      ...
      $activities = FormDetail::where('form_id',$request->params['form_id'])->get();
    ...
    public function addActivity(Request $request)
    {
      ...
      $activity = FormDetail::where('division_id',$request->params['division']['id'])
        ...
        if($activity == null)
        {
            ...
            'area_id'=>$request->params['area'],'quantity'=>'0','percentage_completed'=>'0', 'working_hours'=>'0', 'extra_hours'=>'0']);
    ...
    {
        FormDetail::where('id',$request->activity_id)->update(['quantity'=>$request->quantity,'percentage_completed'=>$request->percentage, 'working_hours'=>$request->working_hours, 
        'extra_hours'=>$request->extra_hours, 'activity_completed'=>$request->activity_completed,'note'=>$request->notes
        ]);
    }
    
    public function deleteActivity(Request $request)
    {
        $res = FormDetail::where('id',$request->activity_id)->delete();
        return response(['result'=>FormDetail::where('id',$request->activity_id)->first()]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                $redirect = redirect("/admin/{$dataType->slug}/{$data->id}/edit");
            } else {
                $redirect = redirect()->back();
            }
